...
public class UserController {
	...
	/*
	 * @RequestMapping(value = "ModifyOk") public String
	 * ModifyOk(@RequestParam("nicname") String nicname,@RequestParam("pwd") String
	 * pwd,@RequestParam("email") String email) { UserDTO userDto = new
	 * UserDTO(nicname, pwd, email, 1, 2); dao.update(userDto);
	 * 
	 * int user_no = dao.userNo(nicname);
	 * 
	 * 
	 * PwDTO pwdto = new PwDTO(user_no, pwd); 수정 더 해야함
	 * 
	 * 
	 * return "Profile"; }
	 */
	...
	public String ModifyOk(HttpSession session, @RequestParam("nicname") String nicname,
			@RequestParam("pwd") String pwd, @RequestParam("email") String email, Model model) {
		String id = session.getAttribute("id").toString();

		UserDTO userDto = new UserDTO(nicname, id, email);
		dao.update(userDto);
